...
cat -Path $outputDirectory/analysis.yaml | `
% {$_ -replace "'@devsnicket/eunice-run-tests-from-file-system'", "run-tests-from-file-system" } `
...
cat $outputDirectory/run-tests-from-file-system.yaml `
| % {$_ -replace "^- id: ", "- id: run-tests-from-file-system\" } `
